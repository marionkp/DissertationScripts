...
# basepath = "/data/mkp42/LLL_Results/"
...
for patient in patients:
    f1 = open(basepath + str(patient) + "/LLL_L_MPRAGE.txt" , "r")
    content1 = f1.read()
    f1.close()
    f2 = open(basepath + str(patient) + "/LLL_R_MPRAGE.txt" , "r")
    content2 = f2.read()
    f2.close()
    row = (str(patient) + " " + content1 + " " + content2).replace('\n', '').replace("  ", ' ').split()
    print(row)
    with open('/data/mkp42/LLL_Results/LLLvolumes.csv', 'a') as file:
